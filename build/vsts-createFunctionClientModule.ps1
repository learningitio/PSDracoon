...
$parentModule = 'PSDracoon'
if (-not $ModuleName) { $ModuleName = 'PSDracoon.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSDracoon'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSDracoon.psd1"
$prereqHash = @{
