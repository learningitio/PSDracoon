...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'PSDracoon.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'PSDracoon.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'PSDracoon.Client.ProtectedToken'
